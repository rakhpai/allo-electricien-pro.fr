/**
 * Generate images for 15 pages using local cached source images
 */
...
console.log('═══════════════════════════════════════════════════════════');
console.log('GENERATE 15 MISSING IMAGES (LOCAL SOURCE)');
console.log('═══════════════════════════════════════════════════════════\n');
...
/**
 * Get source images from cache directory
 */
async function getCachedSourceImages() {
...
async function main() {
  ...
  // Get cached source images
  const sourceImages = await getCachedSourceImages();
  console.log(`📁 Found ${sourceImages.length} cached source images\n`);
  ...
  for (let i = 0; i < PAGES_TO_GENERATE.length; i++) {
    ...
    try {
      ...
      // Use round-robin source image selection
      const sourceImage = sourceImages[i % sourceImages.length];
      const sourcePath = path.join(CONFIG.cacheDir, sourceImage);
